...
void fill_B(int nx, int ny, double hx, double hy, double x0, double y0, double* b, double (*f)(double, double), int p, int pi, int p_mid, double f_abs) {
	int n = (nx + 1) * (ny + 1);
    int i1, i2, i, j, l;
...
void *thread_func(void *args)
{
	...
	int cpu_id = n_cpus - 1 - (arg->pi % n_cpus);
	CPU_SET(cpu_id, &cpu);
	...
	while (!(arg->finish))
	{
		if (!(arg->ready))
		{
			...
			fill_B(nx, ny, hx, hy, a, c, B, f, p, pi, p_mid, f_abs);

			arg->t1 = GetCpuTime();
			arg->it = minimal_residual_msr_matrix_full(n, A, I, B, x, r, u, v, eps, max_it, maxsteps, p, pi);
